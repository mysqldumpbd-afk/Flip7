// ═══════════════════════════════════════════════════════════════
// components.jsx — FLIP 7: Race to 200  v3.0
// Babel transpila en runtime. Cambios v3.0:
//   - PlayerRow rediseñado: 18 emojis zodiaco, 9 colores básicos, dropdowns
//   - CreateRoom: layout ajustado al viewport (flex column, scroll interno)
//   - Badge sala: código más grande y legible
//   - Botón Cero: rojo con shake animation
//   - Revancha: mismo roomCode, mismos jugadores, nueva ronda
//   - API Key Claude: cargada desde Firebase (no expuesta en frontend)
// ═══════════════════════════════════════════════════════════════
...
// 18 animales — 12 zodiaco chino + 6 extras
const EMOJIS=[
  "🐀","🐂","🐅","🐇","🐉","🐍",
  "🐎","🐏","🐒","🐓","🐕","🐖",
  "🦁","🐻","🐼","🦊","🦋","🦅"
...
// 9 colores básicos bien diferenciados
const COLORS=[
...
const LANGS={
  es:{
    ...
    addCard:"➕ AGREGAR CARTA BASE (0-12)",whatNum:"¿QUÉ NÚMERO? (0-12)",
  ...
  en:{
    ...
    addCard:"➕ ADD BASE CARD (0-12)",whatNum:"WHAT NUMBER? (0-12)",
...
// ── PLAYERROW — rediseñado con dropdowns emoji y color ────────
function PlayerRow({idx,name,emoji,color,allEmojis,allColors,usedColors,canRemove,onName,onEmoji,onColor,onRemove,T}){
  ...
  // Posición dinámica del dropdown emoji
  ...
  // Calcular posición del dropdown relativo al botón
  function openEmoji(){
    snd('tap');
  ...
  return(
    <div className="player-setup-row" style={{borderColor:color+"44"}}>
      <div className="psr-top">
        {/* Botón emoji — muestra icono seleccionado en grande */}
        ...
        {/* Input nombre */}
        <input className="inp"
          style={{margin:0,flex:1,borderColor:color+"55",padding:"10px 12px"}}
          placeholder={(T?T.players:"Jugador")+" "+(idx+1)}
          value={name}
          onChange={e=>{snd('num');onName(e.target.value);}}
          onFocus={()=>snd('tap')}/>

        {/* Botón color — muestra color elegido */}
        <div style={{position:"relative",flexShrink:0}}>
          <button ref={colorBtnRef} className="color-dd-btn" onClick={openColor}
            style={{background:color,borderColor:"rgba(255,255,255,.5)"}}>
          </button>
          {colorOpen&&(
            <div ref={colorDdRef} className="color-dd"
              ...
              {allColors.map((c,ci)=>{
                ...
                return(
                  <button key={ci} className={"color-dd-opt"+(sel?" sel":"")+(taken?" taken":"")}
                    ...
                    onClick={()=>{if(!taken){snd('tap');onColor(c);setColorOpen(false);}}}>
                  </button>
                );
              })}
            </div>
...
function App(){
  ...
  // ── Seguridad: aiConfig se llena desde Firebase, no hardcodeado ──
    ...
    async function loadAiConfig(){
      try{
        ...
        setAiConfig({
          ...
          key:gemCfg?.key||"",
          // claudeKey se carga desde Firebase — nunca en el código fuente
          claudeKey:claudeCfg?.key||""
        });
        setAiLoaded(true);
        return;
      }catch(e){console.log("Firebase config fallback to local");}
      try{
  ...
  function leaveGame(){
    ...
    setDemoMode(false);setIsSpectator(false);setMyPlayerId(null);
  }

  // ── REVANCHA: mismo código de sala, mismos jugadores, puntos a 0 ──
  async function startRematch(prevPlayers){
    snd('round');
    winnerShown.current=false;prevSortedRef.current=[];
    const freshPlayers=prevPlayers.map(p=>({...p,total:0,rounds:[]}));
    const currentCode=roomCode||uid4();
    // Actualizar sala existente (no crear nueva)
    await dbRef.current.set("rooms/"+currentCode,{
      code:currentCode,round:1,roundScores:{},finished:false,
      winner:null,createdAt:Date.now(),players:freshPlayers
    });
    setWinner(null);
    setTab("round");
    // Re-suscribir para asegurar que todos reciben el reset
    subscribe(currentCode,dbRef.current);
  }
  ...
  return(
    <div className="wrap">
      ...
      {winner&&<WinnerScreen winner={winner} onClose={()=>{setWinner(null);setTab("scores");}} onRematch={()=>{if(room&&room.players)startRematch(room.players);}} T={T}/>}
    </div>
  );
}
...
function HomeScreen({onEnter,sessions,aiConfig,setAiConfig,lang,setLang,T}){
    ...
    <div className="create-wrap">
      ...
      <div className="create-body">
        ...
        {names.map((n,i)=>(
          <PlayerRow key={i} idx={i} T={T}
            name={n}
            emoji={playerEmojis[i]||EMOJIS[i%EMOJIS.length]}
        ...
        {names.length<18&&(
          <button className="btn-add" onClick={()=>{snd('tap');setNames(p=>[...p,""]);}}
            style={{marginTop:4}}>
            + {T.addPlayer}
          </button>
        )}
      ...
      <div className="create-footer">
        <button className="btn btn-y" style={{marginBottom:8}}
          onClick={()=>{snd('tap');createReal();}}
          disabled={busy||names.filter(n=>n.trim()).length<2}>
  ...
  return(
    <div className="wrap"><div className="page" style={{paddingTop:24}}>
      ...
      {sessions.length>0&&(<>
        <div className="div"/>
        <p className="sec">{T.recentSessions}</p>
        <SesCards sessions={sessions} T={T} onClear={()=>{setSessions([]);try{localStorage.removeItem("f7sess")}catch{}}}/>
      </>)}
      <div style={{textAlign:"center",marginTop:20,paddingBottom:8}}>
        <span style={{fontFamily:"'Righteous',sans-serif",fontSize:".6rem",color:"rgba(255,255,255,.15)",letterSpacing:2}}>v3.0 · FLIP 7</span>
        <button onClick={()=>{localStorage.clear();sessionStorage.clear();window.location.reload();}}
          style={{display:"block",margin:"6px auto 0",background:"none",border:"none",
            color:"rgba(255,255,255,.15)",fontFamily:"'Righteous',sans-serif",
            fontSize:".58rem",cursor:"pointer",letterSpacing:1,textDecoration:"underline"}}>
...
// ── ROUNDTAB — con botón Cero rojo y animación shake ─────────
function RoundTab({room,allDone,onSubmit,onUndo,onFinalize,myPlayerId,demoMode,aiConfig,setAiConfig,onRematch,onEndGame,roomCode,T}){
  ...
  function handleZero(pid){
    snd('zero');
    setShakePid(pid);
    setTimeout(()=>setShakePid(null),500);
    onSubmit(pid,0,"zero");
  }
    ...
    <>
      ...
      <div className="rsb" style={{marginBottom:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {/* Código de sala más visible en round */}
          ...
          <div key={p.id} className={"sc "+cls+" "+(p._moved==='up'?'moved-up':p._moved==='down'?'moved-down':'')}>
            ...
            <div style={{flex:1}}>
              <div style={{fontWeight:900,fontSize:".98rem",display:"flex",alignItems:"center",gap:6}}>
                ...
                <span style={{color:p.color,textShadow:"0 0 10px "+p.color+"44"}}>{p.name}</span>
              </div>
              ...
              return(
                <div key={p.id} className={p._moved==='up'?'moved-up':p._moved==='down'?'moved-down':''}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:900,fontSize:nameSize,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.emoji} {p.name}</div>
          ...
          <div key={s.id} className="hc" style={{cursor:"pointer"}} onClick={()=>{snd('tap');setExpanded(isOpen?null:s.id);}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div>
                ...
                <div style={{fontSize:".7rem",color:"rgba(255,255,255,.32)",fontWeight:700}}>
                  {s.players.length} jugadores · {s.rounds} rondas · Sala {s.code}
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              <div style={{marginTop:12}}>
                {sorted.map((p,i)=>(
                  <div key={p.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,.05)"}}>
                    <div style={{fontFamily:"'Anton',sans-serif",fontSize:"1.4rem",color:i===0?"var(--y)":"rgba(255,255,255,.3)",width:28}}>
                      {i===0?"🥇":i===1?"🥈":i===2?"🥉":"#"+(i+1)}
                    </div>
                      ...
                      <tbody>
                        {sorted.map(p=>(
                          <tr key={p.id}>
                            ...
                            {(p.rounds||[]).map((r,i)=><td key={i} style={{textAlign:"center",padding:"4px",color:r.score===0?"rgba(255,255,255,.3)":"rgba(255,255,255,.7)"}}>{r.score===0?"💀":r.score}</td>)}
...
// ── SCANMODAL — API Key Claude viene de Firebase, no del código ─
    ...
    try{
      ...
      if(provider==="gemini"){
        ...
        for(var mi=0;mi<MODELS.length;mi++){
          var model=MODELS[mi];
          try{
            var body={contents:[{parts:[{inline_data:{mime_type:mime,data:b64}},{text:ACTIVE_PROMPT}]}]};
        ...
        // ── Claude: usa claudeKey cargada desde Firebase, no hardcodeada ──
      ...
      React.createElement("div",{className:"ms"},
        ...
        phase==="pick"&&React.createElement(React.Fragment,null,
          React.createElement("div",{style:{background:"rgba(46,196,182,.08)",border:"1px solid rgba(46,196,182,.22)",borderRadius:12,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10}},
            React.createElement("div",{style:{flex:1}},
              React.createElement("div",{style:{fontWeight:900,fontSize:".82rem",color:"var(--t)"}},apiKey?"✅ Gemini 2.5 Flash - Activo":"⏳ Cargando configuración..."),
          ...
          showKeyEdit&&React.createElement("div",{className:"api-box",style:{marginBottom:12}},
            React.createElement("p",null,"Cambiar IA o key (opcional):"),
        ...
        phase==="preview"&&React.createElement(React.Fragment,null,
          ...
          React.createElement("div",{style:{background:"rgba(46,196,182,.08)",border:"1px solid rgba(46,196,182,.2)",borderRadius:10,padding:"8px 12px",marginBottom:8,textAlign:"center"}},
            React.createElement("span",{style:{fontFamily:"'Righteous',sans-serif",fontSize:".68rem",color:"var(--t)",letterSpacing:1}},"⚡ Modo: solo números base · Ajusta chips después del scan")
          ),
        ...
        phase==="error"&&React.createElement(React.Fragment,null,
          React.createElement("div",{style:{textAlign:"center",padding:"20px 0"}},
            ...
            (errMsg.includes("404")||errMsg.includes("no encontrado"))&&React.createElement("div",{style:{background:"rgba(46,196,182,.08)",border:"1px solid rgba(46,196,182,.2)",borderRadius:10,padding:"10px 14px",fontSize:".8rem",fontWeight:800,color:"var(--t)"}},"Ve a aistudio.google.com → Claves de API → Crea una nueva key")
          ),
          React.createElement("div",{className:"mr2"},
  ...
  return(
    React.createElement(React.Fragment,null,
      React.createElement("div",{style:{marginBottom:10}},
        React.createElement("div",{style:{fontFamily:"'Righteous',sans-serif",fontSize:".65rem",color:"rgba(255,255,255,.35)",letterSpacing:3,marginBottom:8,textAlign:"center"}},"CARTAS DETECTADAS - toca ✕ para quitar"),
        React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center",minHeight:48,alignItems:"center"}},
          ...
          cards.map((n,i)=>React.createElement("div",{key:i,style:{background:CARD_BG_MAP[n]||"#F6EFD9",border:"3px solid rgba(0,0,0,.12)",borderRadius:10,padding:"6px 10px",display:"flex",alignItems:"center",gap:6,boxShadow:"0 3px 8px rgba(0,0,0,.4)",position:"relative",minWidth:52,justifyContent:"center"}},
            ...
            React.createElement("button",{onClick:()=>removeCard(i),style:{position:"absolute",top:-8,right:-8,width:20,height:20,background:"var(--r)",border:"none",borderRadius:"50%",color:"white",fontSize:".7rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,lineHeight:1}},"✕")
          ))
        )
      ),
      ...
      !adding?React.createElement("button",{onClick:()=>{snd("tap");setAdding(true);setNewVal("");},style:{width:"100%",background:"rgba(255,255,255,.06)",border:"2px dashed rgba(255,255,255,.2)",borderRadius:12,padding:"10px",cursor:"pointer",fontFamily:"'Righteous',sans-serif",fontSize:".78rem",color:"rgba(255,255,255,.5)",letterSpacing:1,marginBottom:12}},"➕ AGREGAR CARTA BASE (0-12)")
      :React.createElement("div",{style:{background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.12)",borderRadius:12,padding:"12px",marginBottom:12}},
        React.createElement("div",{style:{fontFamily:"'Righteous',sans-serif",fontSize:".62rem",color:"rgba(255,255,255,.35)",letterSpacing:2,marginBottom:8,textAlign:"center"}},"¿QUÉ NÚMERO? (0-12)"),
...
// ── WINNERSCREEN — revancha sin salir ─────────────────────────
function WinnerScreen({winner,onClose,onRematch,T}){
  ...
  return(
    React.createElement("div",{className:"wb"},
      ...
      // REVANCHA — no saca a los jugadores, resetea puntos en la misma sala
      React.createElement("button",{className:"btn btn-y",onClick:()=>{snd("round");onRematch();},style:{maxWidth:300,marginBottom:10}},
        "🔁 REVANCHA · Mismos jugadores"
      ),
      React.createElement("button",{className:"btn btn-g",onClick:()=>{snd("tap");onClose();},style:{maxWidth:300}},
        "📊 Ver marcador final"
      )
    )
  );
